...
static unsigned int isoless_init()
{
    ...
    else if (streq(camera_model_short, "650D"))
    {
		...
		uint8_t  first=1;
		for(uint i=0;i< PHOTO_CMOS_ISO_COUNT;i++)
		{
			uint16_t raw=*(uint16_t *)(photo_cmos_start_set[0]+i*PHOTO_CMOS_ISO_SIZE);
		...
		console_printf("ph_addr:%x\n",PHOTO_CMOS_ISO_START);
#endif
        CMOS_ISO_BITS = 3;
        CMOS_FLAG_BITS = 2;
        CMOS_EXPECTED_FLAG = 3;
